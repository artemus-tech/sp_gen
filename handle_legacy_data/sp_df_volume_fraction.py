...
if __name__ == '__main__':
    ...
    sql_result = dbassets.get_fields_data(conn, "generate_results",["id","src_path", "r","rmin", "rmax"])
    ...
    for row in sql_result:
        ...
        rglobal = float(row['r'])
        rmax = float(row['rmax'])
        data = np.loadtxt(f'C:\\{row["src_path"]}')
        n=10

        rmiddle = float(np.array([el for el in data[:, 3]]).mean())

        dbassets.update_field_double(conn, field = "rmean",table=constants.tables_dict["gen"],id=row['id'], value=rmiddle)
        conn.commit()
        arg, func = get_density_df(rglobal + rmax, n, data)

        phi_by_five_sp_layers=float(np.mean(func[:5]))
        dbassets.update_field_double(conn, field = "phi_five_splayers",table=constants.tables_dict["gen"],id=row['id'], value=phi_by_five_sp_layers)
        conn.commit()
        ...
        plt.savefig(f"C:\sp-df-volume-fraction\\{row['id']}.png")
        np.savetxt(f"C:\sp-df-volume-fraction\\{row['id']}.txt", np.c_[arg, func])
    conn.close()